...
export default function Event(props){
  ...
  return(
    <View style={styles.card}>
      ...
      >
        <Image style={styles.profileImage} source={require(profileImage)}/>
        <View style={styles.userNameBox}>
          <Text style={styles.userName}>{data.userName}</Text>
        </View>
      </TouchableOpacity>
      <TouchableOpacity style={styles.remainingPost} onPress={() => toPost(data.id)}>
        <Image style ={styles.mainImage} source={{uri:data.image}}/>
        <View style={styles.detailSection}>
          <Text style={styles.eventName}>{data.eventName}</Text>
          <View style={styles.dateLocationContainer}>
            <Text style={styles.boldGray}>{stringifyTimestamp(data.eventTime)}</Text> 
          ...
          <View style={styles.bottomBar}>
            <Text style={styles.attendeeCount}>{data.followers.length} attending</Text>
            {data.canDelete?
              <TouchableOpacity style={styles.deleteButton} onPress={() => onDelete(data.id)}>
                <Text style={styles.buttonText}>Delete</Text>
              </TouchableOpacity>
              :
              <TouchableOpacity style={styles.messageButton} onPress={() => toChat()}>
...
const styles = StyleSheet.create({
  card: { //The whole box the post is in
  ...
  userSection: { //The bit at the top with the poster and their pfp
    flex:2,
    flexDirection: "row",
  ...
  userNameBox: { //Container with just the user name <Text> in it
    flex:16
  },
  userName: {
    fontWeight: "bold",
  ...
  detailSection: { //The part under the image with all the details
    flex:5,
    paddingTop: 5,
    paddingLeft: 15,
    paddingRight: 15
